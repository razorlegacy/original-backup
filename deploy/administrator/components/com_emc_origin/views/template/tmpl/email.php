...
<?php
	...
	$subject	= '[Origin] Project Details - '.$this->originObj->name.'[Origin ID: '.$this->originObj->id.']';
	$body		= '<p>Details for Origin Project #'.$this->originObj->id.': <strong>'.$this->originObj->name.'</strong></p>';
	$body		.= '<h3>Embed Code</h3>';
	$body		.= '<p style="font-style: italic">Use the "Evolve Origin" DFP template.</p>';
	$body		.= '<blockquote style="background-color: #ebebeb; padding: 5px;">'.htmlentities($this->emailObj['origin_list_embed_code']).'</blockquote>';
	...
	$body		.= '<a href="'.$this->emailObj['previewLink'].'">'.$this->emailObj['previewLink'].'</a>';
