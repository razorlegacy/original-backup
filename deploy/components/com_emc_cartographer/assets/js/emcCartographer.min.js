...
/* Cartographer */
var loadLive,emcCartographerTooltip={events:{hide:function(b,a){$j(a.elements.tooltip).find(".ui-tooltip-content").hide().html("").show()},show:function(b,a){if(loadLive&&$j(a.elements.target).data("link"))b.preventDefault(),window.open($j(a.elements.target).data("link"),"");else{a.set("content.text",$j(a.elements.target).data("content").content);var c=$j(a.elements.tooltip).height(),d=$j(a.elements.titlebar).height(),e=$j(a.elements.content).css("max-height");c-d-10+"px">=e&&$j(a.elements.content).slimScroll()}}},
hide:{event:"click unfocus",fixed:!0},position:{adjust:{method:"shift"},at:"center center",effect:!1,my:"top left"},show:{event:"click",solo:!0},style:{def:!1}};
emcCartographer={init:function(){loadLive=!0;emcCartographer.qtip();emcCartographer.analytics("load","");emcCartographerGroups.init()},analytics:function(b,a){switch(b){case "load":_gaq.push(["_trackEvent","[Cartographer] "+emcCartographer_id+"-"+emcCartographer_name,"[Load]"]);break;case "tooltip":_gaq.push(["_trackEvent","[Cartographer] "+emcCartographer_id+"-"+emcCartographer_name,"[Tooltip] "+a])}},qtip:function(){$j(".emcCartographer_marker").each(function(){$j(this).data("link");$j(this).qtip($j.extend(!0,
{},emcCartographerTooltip,{content:{text:$j(this).data("content").content,title:{button:!0,text:"<h4>"+$j(this).data("content").title+"</h4>"}},id:"emcCartographer_tooltip_"+$j(this).data("id"),position:{target:"event",viewport:$j("#emcCartographer_markers_"+$j(this).parent().data("id"))},show:{event:cartographerMarkerTrigger},style:{classes:"emcCartographer_tooltip emcCartographer_tooltip_"+$j(this).parent().data("id")+" emcCartographer_tooltip_"+$j(this).data("content").tooltip_size_type}}))})}};
emcCartographerGroups={init:function(){emcCartographerGroups.tabs()},tabs:function(){$j("#emcCartographer_group_list li").click(function(){$j(".emcCartographer_group").hide();$j("#"+$j(this).data("group")).show();return!1})}};