<?php
	class templateUtils {
		...
		public function createModule($position) {
			$output		= "";
			
			if($this->countModules($position)) {
				$output	.= "<div id='{$position}'>";
				$output		.= $this->get_module($position, 'springboard');
				$output	.= "</div>";
			}			
			return $output;
		}

		public function get_module($position, $style = "xhtml", $params=array()) {
			if ($this->errorpage) {
				$this->get_module_force($position, $style);
			} else {
				return '<jdoc:include type="modules" style="'.$style.'" name="'.$position.'" />';
			}	
		}
