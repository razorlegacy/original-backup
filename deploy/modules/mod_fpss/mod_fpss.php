...
$mosConfig_live_site = $mainframe->isAdmin() ? $mainframe->getSiteURL() : $config->getValue('cdn_url');;
...
if($exist) {
	...
	// FIND WHAT ITEM LINKS WE CAN SEE
	$query = "SELECT itemlink FROM "._FPSS_TABLE_SLIDES." WHERE itemlink!=0";
	$db->setQuery( $query );
	$slide_items = $db->loadObjectList();
	...
	foreach ($slide_items as $key => $slide_item) {
		$item_links[] = $slide_item->itemlink;
	}
	$content_id_list = implode(',', $item_links);
	...
	/* ----------- Movie carousel specific stuff ------------------- */
	if ($fpss_template == 'carousel') {
    echo '<!-- styling -->
          <link rel="stylesheet" type="text/css" href="/modules/mod_fpss/mod_fpss/templates/carousel/movie_carousel.css" />
          
          <!-- ***** Movie carousel starts here ***** -->
	...
	/* --------- End Movie Carousel specific stuff ----------------- */
    ...
    if($fpss_template == 'carousel'){
      echo '<div class="item_container">
              <a href="'.$link.'" class="ic_link">
                <img src="'.$mosConfig_live_site.'/'.$image->path.'" />
              </a>
              <div style="display:none">'.$slidetitle.' ('.$image->plaintext.')</div>
            </div>';
    }
    else{
		/* --------------------- SLIDE OUTPUT --------------------- */
  ...
  /* ----------- Movie carousel specific stuff ------------------- */
	if ($fpss_template == 'carousel') {
	  echo '    </div>	 
          ...
          <!-- ***** Movie carousel ends here ***** -->          
          <script src="/modules/mod_fpss/mod_fpss/templates/carousel/jquery.tools.min.js"></script>
          <script src="/modules/mod_fpss/mod_fpss/templates/carousel/jquery.mousewheel.js"></script>
          <script src="/modules/mod_fpss/mod_fpss/templates/carousel/movie_carousel.js"></script>';	
	}
	/* --------- End Movie carousel specific stuff ----------------- */
...
<link type="text/css" media="all" rel="stylesheet" href="'.$mosConfig_live_site.'/modules/mod_fpss/mod_fpss/templates/'.$fpss_template.'/template_css.php?w='.$width.'&h='.$height.'&sw='.$sidebar_width.'" />
';
	
	if (!$hide_nav) {
	$modFPSShead .= '
<!--[if lte IE 7]>
<link type="text/css" media="all" rel="stylesheet" href="'.$mosConfig_live_site.'/modules/mod_fpss/mod_fpss/templates/'.$fpss_template.'/template_css_ie.css" />
<![endif]-->
	';
	}
