			...
			if ($thumb_embed && $counter) {
				...
				if (!empty($txtimg[0])) {
					foreach ($txtimg[0] as $txtimgel) {
						...
						if (strpos("http",$txtimgel) === 0) {
							preg_match("#src=\"/?$imgregex\"#",$txtimgel,$txtimgelsr);
						} elseif ( strstr($txtimgel, $image_path) ) {
							preg_match("#src=\"/?$image_path/$imgregex\"#",$txtimgel,$txtimgelsr);
						}

						if (!empty($rows[$r]->images) && isset($txtimgelsr[1])) {
							$rows[$r]->images = $txtimgelsr[1] . "\n" . $rows[$r]->images;
						} else if (isset($txtimgelsr[1])) {
							$rows[$r]->images = $txtimgelsr[1];
						}
					}
				}
...
function fptn_calcsize($srcx, $srcy, &$forcedwidth, &$forcedheight, $aspect) {
	...
	if (empty($srcx) || empty($srcy) || empty($forcedwith) || empty($forcedheight) || empty($aspect)) return;
