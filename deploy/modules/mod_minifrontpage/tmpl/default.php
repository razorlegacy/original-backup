...
if ( $num_intro ) { 
	if ($col==0) { ?><tr><?php }?> 
		...
		if ($loadorder == 0) { 
			...
			if(strlen($row->introtext) <= $limit) {
				echo $row->introtext; 
			}
			
			// is $break present between $limit and the end of the string?  
			if(false !== ($breakpoint = strpos($row->introtext, ".", $limit))) { 
				if($breakpoint < strlen($row->introtext) - 1) { 
					$row->introtext = substr($row->introtext, 0, $breakpoint) . "..."; 
				} 
			} 
