...
/**
* Sweepstakes Web Service JS
*/
jQuery(function(){$(".websvc_rules").colorbox({iframe:true,width:"50%",height:"80%"});dateCheck();$("#websvc_form_"+websvc_sid).submit(function(){if($("#websvc_form_"+websvc_sid).validationEngine({returnIsValid:true,promptPosition:"centerRight"})&&entrantCheck()){var a=$("#websvc_form_"+websvc_sid).serialize();$.ajax({url:"http://webservices.evolvemediacorp.com/index.php?format=raw",data:a,dataType:"jsonp",jsonp:"jsonCB",jsonpCallback:"validation"})}return false})}); function validation(a){$("span[id*=websvc_error_]").each(function(){$(this).remove()});$("#websvc_form_"+websvc_sid+" *").removeClass("websvc_error");if(a.length){var b=a.length>1?"Please check the following fields":"Please check the following field";$("#websvc_response").html(b);$("#websvc_response").removeClass("websvc_hidden");for(var c in a)$("#"+a[c]).parent().find("label").addClass("websvc_error")}else{$("#websvc_response").removeClass("websvc_hidden");$("#websvc_response").html("Thank you for entering!"); websvc_me==0&&entrantSave();formWipe()}}function formWipe(){$(":input","#websvc_form_"+websvc_sid).not(":button, :submit, :reset, :hidden").val("").removeAttr("checked").removeAttr("selected")}function dateCheck(){websvc_endDate=websvc_endDate.replace(/-/g,"/");var a=new Date(websvc_endDate);new Date>a&&$("#websvc_form_"+websvc_sid).fadeOut(0,function(){$("#websvc_closed").removeClass("websvc_hidden")})} function emailFind(){var a;$("#websvc_form_"+websvc_sid+" li").each(function(b){if($(this).text().toLowerCase().replace(/[^a-z0-9 ]/g,"")=="email"){a=b;return false}});return $("#websvc_form_"+websvc_sid+" input").eq(a).attr("id")} function entrantCheck(){var a="websvc_"+websvc_sid;if(websvc_me==0){a=$.cookie(a)==null?"":$.cookie(a);if(a=="")return true;else{var b=emailFind();b=$("#"+b).val();c_emails=a.split(",");if(jQuery.inArray(xor_str(b,5),c_emails)>=0){$("#websvc_response").removeClass("websvc_hidden");$("#websvc_response").html("Thank you for entering!");formWipe();return false}else return true}}else return true} function entrantSave(){var a=emailFind(),b=$("#"+a).val();a="websvc_"+websvc_sid;var c=new Date(websvc_endDate.replace(/-/g,"/")),d=$.cookie(a)==null?"":$.cookie(a);if(d=="")$.cookie(a,xor_str(b,5),{path:"/",expires:c});else{b=xor_str(b,5);$.cookie(a,d+","+b,{path:"/",expires:c})}}function xor_str(a,b){var c="";for(i=0;i<a.length;++i)c+=String.fromCharCode(b^a.charCodeAt(i));return c};